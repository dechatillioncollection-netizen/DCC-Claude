...
| Leader 🫡 | Only slot 0 attacks; slots 1+ add on-hit mods via `leaderMods()`/`applyLeaderMods`. Dmg multipliers: assassin ×1.6, engineer ×1.25, cannon ×1.15 (+splash 120); mage splash 80; priest 2% heal/hit; knight +30% slow; ice wizard +20% slow **and converts hits to ICE**; fire wizard burn **and converts hits to FIRE** (later slot's element wins); ranger = hunt fast/flying; archer grants nothing. On-hit riders only proc for single-target leaders (archer/ranger/assassin); the stat cards show the full readout per slot. |
